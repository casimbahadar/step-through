...
/* Extract CORE from the SHIPPED html and test exactly what ships. */
const html = fs.readFileSync(new URL('./polyglot.html', import.meta.url), 'utf8');
const m = html.match(/\/\* CORE-START \*\/([\s\S]*?)\/\* CORE-END \*\//);
if (!m) { console.error('CORE block not found in polyglot.html'); process.exit(1); }
const POLYGLOT = new Function(m[1] + '\nreturn POLYGLOT;')();
const { parse, print, run, execute, translate, stripPos, fmt, LangError } = POLYGLOT;
console.log('CORE extracted from polyglot.html: ' + m[1].length + ' bytes');
...
console.log('\n=== SHIPPED FILE HARNESS (polyglot.html) ===');
